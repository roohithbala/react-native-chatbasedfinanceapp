...
class SocketService {
  ...
  onTypingStart(callback: (data: { groupId: string; user: any }) => void) {
    if (this.socket) {
      this.socket.on('user-typing-start', (data: any) => {
        callback(data);
      });
    }
  }
  ...
  onTypingStop(callback: (data: { groupId: string; user: any }) => void) {
    if (this.socket) {
      this.socket.on('user-typing-stop', (data: any) => {
        callback(data);
      });
    }
  }
  ...
  onExpenseUpdate(callback: (data: any) => void) {
    if (this.socket) {
      this.socket.on('expense-update', callback);
    }
  }
  ...
  onBudgetUpdate(callback: (data: any) => void) {
    if (this.socket) {
      this.socket.on('budget-update', callback);
    }
  }

  onSplitBillUpdate(callback: (data: any) => void) {
    if (this.socket) {
      this.socket.on('split-bill-update', callback);
    }
  }
